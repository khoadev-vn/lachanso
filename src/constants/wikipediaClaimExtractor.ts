...
},
{
  ...
  aliases: ["donald trump", "donal trump", "president donald trump", "tá»•ng thá»‘ng donald trump"],
  wikiTitle: "Donald Trump",
  validNames: ["donald john trump", "donald trump"]
},
{
...
function resolveKnownProfile(text: string): KnownPersonProfile | null {
  ...
  return KNOWN_PERSON_PROFILES.find((profile) => profile.aliases.some((alias) => normalized.includes(normalizeClaimText(alias)))) ?? null;
}
function extractLeadingPersonName(text: string): string | null {
  const match = text.match(/^([\p{Lu}][\p{L}.'-]+(?:\s+[\p{Lu}][\p{L}.'-]+){1,6})(?:\s+là(?:\s|$)|[,，])/u);
  return match?.[1] ?? null;
}
function cleanClaimedName(name: string): string {
  return compactWhitespace(name.
  replace(/\([^)]*\)/g, " ").
  replace(/^(là|với tên gọi|tên gọi là)\s+/iu, "").
  replace(/\s+cũng\s+như\s+/giu, " ").
  replace(/\s+còn\s+được\s+gọi\s+.*$/iu, ""));
}
...
export function extractWikipediaClaims(text: string): WikipediaExtractedClaims | null {
  ...
  const hasProfileSignals = Boolean(personName) && /\b(is|was|served|president|party|rapper|actor|singer|songwriter|businessman|politician|tong thong|chinh tri gia|doanh nhan|dang|ca si|nhac si|dien vien|nghe danh|nha san xuat|nguoi viet nam|nguoi my|phu nhan)\b/i.test(normalizedCompacted);
  return {
    personName,
...
function hasPresidentOrdinalEvidence(value: number, normalizedEvidence: string): boolean {
  return normalizedEvidence.includes(`tong thong thu ${value}`) ||
...
export function compareIdentityClaimsWithWikipedia(identityClaim: WikipediaIdentityClaim, wikipediaTitle: string, wikipediaExtract: string): WikipediaIdentityComparison {
...
export function compareProfileClaimWithWikipedia(inputText: string, wikipediaTitle: string, wikipediaExtract: string): WikipediaProfileComparison {
  ...
  const extractSentences = wikipediaExtract.split(/(?<=[.!?\n])\s+/).map((s) => normalizeClaimText(s));

  const activeSignals = PROFILE_SIGNAL_GROUPS.filter((group) => group.input.some((signal) => normalizedInput.includes(normalizeClaimText(signal))));
  ...
  for (const ordinal of presidentOrdinals) {
    ...
    const hasValidSentence = extractSentences.some((sentence) =>
    hasPresidentOrdinalEvidence(ordinal, sentence) ||
    sentence.includes(String(ordinal)) && sentence.includes("president")
    );
  ...
  for (const range of yearRanges) {
    ...
    const hasValidSentence = extractSentences.some((sentence) => {
      return sentence.includes(startYear) && sentence.includes(endYear);
    });
    ...
    {
      mismatchedSignals.push(`term ${range}`);
    }
  }
...
export type StatusClaimType = "retired" | "deceased" | "active";
export interface WikipediaStatusClaim {
  type: StatusClaimType;
  label: string;
  phrase: string;
}
export interface WikipediaStatusComparison {
...
const STATUS_CLAIM_PATTERNS: Array<{
  type: StatusClaimType;
  label: string;
  pattern: RegExp;
}> = [
{
  type: "retired",
  label: "giải nghệ",
  pattern: /(?:đ[ãa]|vừa|chính thức)?\s*(giải nghệ|treo giày|tuyên bố giải nghệ|từ giã sự nghiệp)|(?:retired|retirement|hung up (?:his|her) boots)/iu
},
{
  type: "deceased",
  label: "qua đời",
  pattern: /(?:đ[ãa]\s+mất|vừa\s+mất|bất ngờ\s+mất|đột ngột\s+mất|đ[ãa]\s+qua đời|vừa\s+qua đời|từ trần|tạ thế|mất\s+(?:năm|ngày|tại|vào|ở))|(?:died|has died|passed away|death of)/iu
},
{
  type: "active",
  label: "vẫn thi đấu",
  pattern: /(?:vẫn|đang|hiện) (?:còn )?(?:thi đấu|khoác áo|chơi bóng)|(?:still playing|currently plays|active player|is still active)/iu
}];

const STATUS_CLAIM_EVIDENCE: Record<StatusClaimType, {
  supports: string[];
  contradicts: string[];
}> = {
  retired: {
...
export function extractStatusClaims(text: string): WikipediaStatusClaim[] {
  const claims: WikipediaStatusClaim[] = [];
  for (const item of STATUS_CLAIM_PATTERNS) {
    const match = text.match(item.pattern);
    if (match?.[0]) {
      claims.push({
        type: item.type,
        label: item.label,
        phrase: compactWhitespace(match[0])
      });
    }
  }
  return unique(claims.map((claim) => claim.phrase).length > 0 ? claims : []);
}

function containsStatusEvidence(signals: string[], normalizedEvidence: string): boolean {
  return signals.some((signal) => normalizedEvidence.includes(normalizeClaimText(signal)));
}

export function compareStatusClaimWithWikipedia(claim: WikipediaStatusClaim, wikipediaExtract: string): {
  supports: boolean;
  contradicts: boolean;
  unknown: boolean;
} {
  if (!wikipediaExtract) {
    return { supports: false, contradicts: false, unknown: true };
  }
  const normalizedExtract = normalizeClaimText(wikipediaExtract);
  const evidence = STATUS_CLAIM_EVIDENCE[claim.type];
  let supports = containsStatusEvidence(evidence.supports, normalizedExtract);
  let contradicts = containsStatusEvidence(evidence.contradicts, normalizedExtract);
  if (claim.type === "deceased") {
    const lifespanRange = wikipediaExtract.match(/\b\d{1,2}\s+\w+\s+\d{3,4}\s*[–—-]\s*\d{1,2}\s+\w+\s+\d{3,4}\b|\b\d{3,4}\s*[–—-]\s*\d{3,4}\b/);
    if (lifespanRange) {
      supports = true;
    }
  }
  if (supports && !contradicts) {
    return { supports: true, contradicts: false, unknown: false };
  }
  if (contradicts && !supports) {
    return { supports: false, contradicts: true, unknown: false };
  }
  return { supports: false, contradicts: false, unknown: true };
}

export function compareStatusClaimsWithWikipedia(claims: WikipediaStatusClaim[], wikipediaExtract: string): WikipediaStatusComparison {
  const matchedClaims: string[] = [];
  const contradictedClaims: string[] = [];
  const unknownClaims: string[] = [];
  for (const claim of claims) {
    const result = compareStatusClaimWithWikipedia(claim, wikipediaExtract);
    if (result.supports) {
      matchedClaims.push(claim.label);
    } else if (result.contradicts) {
      contradictedClaims.push(claim.label);
    } else {
      unknownClaims.push(claim.label);
    }
  }
  return {
    matchedClaims,
    contradictedClaims,
    unknownClaims,
    verified: contradictedClaims.length === 0 && matchedClaims.length > 0
  };
}
