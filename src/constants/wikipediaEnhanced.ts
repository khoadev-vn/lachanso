/**
 * ENHANCED WIKIPEDIA INTEGRATION - Lá Chắn Số
 * Tích hợp Wikipedia Knowledge Graph và Wikidata
 * Hỗ trợ entity linking, structured data extraction, fuzzy matching
 */
...
/**
 * Fetch Wikipedia profile with caching
 */
export async function getWikipediaProfile(personName: string): Promise<WikipediaProfile | null> {
  ...
  try {
    ...
    const profile: WikipediaProfile = {
      title: page.title,
      extract: (page.extract || "").substring(0, 500), // First 500 chars
      pageUrl: `https://en.wikipedia.org/wiki/${encodeURIComponent(pageTitle)}`,
    };

    // Try to get Wikidata info for this person
...
/**
 * Get Wikidata claims for an entity
 */
export async function getWikidataClaimsForEntity(entityName: string): Promise<WikidataClaim[]> {
  ...
  try {
    ...
    const entityId = entities[0].id; // e.g., Q76
    ...
    for (const [propertyId, propertyName] of Object.entries(propertyMap)) {
      ...
      if (claimList && claimList.length > 0) {
        ...
        } else if (mainSnak?.datatype === "wikibase-entityid") {
        ...
        } else if (mainSnak?.datatype === "string") {
 ...
 * Returns potential entity matches
 */
export async function linkEntitiesToWikidata(text: string): Promise<EntityMatch[]> {
  ...
  // Search top entities only (limit to 5 for performance)
...
/**
 * Compare extracted facts with Wikipedia/Wikidata
 */
export async function compareFactsWithWikipedia(
  personName: string,
...
/**
 * Batch fetch Wikipedia profiles
 */
