...
import { parallelProcess, withTimeout, hashClaim, RateLimiter } from "../utils/performanceOptimizer";
...
export function extractClaimsFromText(text: string): ClaimExtractionResult[] {
  ...
  for (const sentence of sentences) {
    ...
    {
      pattern: /\b(said|claimed|stated|revealed|announced)\b/i,
      category: "direct_claim",
      confidence: 0.7
    },
    {
      pattern: /\b(is|was|are|were)\s+([a-zA-ZÀ-ỹ\s]+?)\b/i,
      category: "identity",
      confidence: 0.8
    },
    {
      pattern: /\b(\d+)\s*(percent|%|million|billion|thousand)\b/i,
...
export function rankFactCheckResults(results: FactCheckResult[], originalClaim: string): FactCheckResult[] {
  return results.
  map((result) => ({
    ...result,
...
function mapFactCheckRating(rating?: string): FactCheckResult["rating"] {
  ...
  if (lower.includes("true") || lower.includes("correct"))
  return "true";
  if (lower.includes("false") || lower.includes("incorrect"))
  return "false";
  if (lower.includes("mixed") || lower.includes("partly"))
  return "mixed";
  return "unverified";
}
...
export async function crossVerifyClaim(claim: string): Promise<{
  ...
  if (falseCount > trueCount && trueCount === 0) {
    consensus = "false";
  } else
