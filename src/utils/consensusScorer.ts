/**
 * CONSENSUS SCORER - Lá Chắn Số
 * Multi-source consensus logic with weighted source authority
 * Implements context-aware temporal analysis and manipulation detection
 */
...
export interface VerificationSource {
  ...
  weight: number; // Authority weight
}
...
export interface ConsensusVerification {
  ...
  temporalRelevance: number; // 0-1, relevance based on date
}
...
export function calculateConsensus(sources: VerificationSource[]): ConsensusVerification {
  ...
  } else if (normalizedFalse > 0.6 && normalizedVerified < 0.2) {
    finalVerdict = "false";
  } else if (normalizedMixed > 0.4 || (normalizedVerified > 0.3 && normalizedFalse > 0.3)) {
    finalVerdict = "mixed";
  }
  ...
  const agreementScore = maxVerdictScore; // Higher = more agreement
  ...
  return {
    ...
    manipulationScore: 0, // Filled in by detectManipulation
    temporalRelevance: 0, // Filled in by temporalAnalysis
  };
}
...
export function detectManipulation(text: string): {
  score: number;
  patterns: Array<{ name: string; severity: number }>;
} {
  const patterns = [
    {
  ...
  let totalScore = 0;
  const detectedPatterns: Array<{ name: string; severity: number }> = [];
...
/**
 * Temporal analysis: check if claim aligns with current events
 */
export function temporalAnalysis(
  ...
  // Extract temporal references in claim
  const datePatterns = [
    /\b\d{4}\b/, // Year
  ...
  if (!hasTemporalRef) {
    // No temporal reference = claim should be timeless
    relevance = 1;
  } else if (ageInDays <= 7) {
    relevance = 1; // Very recent = relevant
 ...
 * More specific claims are easier to verify/falsify
 */
export function claimSpecificity(claim: string): {
  ...
  let specificity = 0;
  if (hasNumbers) specificity += 0.25;
  if (hasNames) specificity += 0.25;
  if (hasLocation) specificity += 0.25;
  if (hasDate) specificity += 0.25;
...
/**
 * Weighted multi-source verification
 */
export function verifyWithWeights(
  wikipediaProfile: WikipediaProfile | null,
  factCheckResults: FactCheckResult[],
  pressArticles: NewsArticle[],
  claim: string
  ...
  if (wikipediaProfile) {
    sources.push({
      type: "wikipedia",
      verdict: "verified", // Assuming Wikipedia profile found = verified
      confidence: 0.8,
  ...
  if (pressArticles.length > 0) {
    ...
    sources.push({
      ...
      weight: avgCredibility / 100, // Variable based on source quality
    });
  }
...
export function combineAnalyses(
  ...
  manipulation: ReturnType<typeof detectManipulation>,
  temporal: ReturnType<typeof temporalAnalysis>,
  specificity: ReturnType<typeof claimSpecificity>
): ConsensusVerification {
  // Adjust confidence based on manipulation and temporal analysis
  const manipulationPenalty = manipulation.score * 0.3; // Manipulation reduces confidence
  const temporalBoost = temporal.relevance * 0.1; // Temporal relevance increases confidence
  const specificityBoost = specificity.score * 0.1; // Specificity increases confidence

  const adjustedConfidence = Math.min(
    Math.max(consensus.confidence - manipulationPenalty + temporalBoost + specificityBoost, 0),
    1
  );
