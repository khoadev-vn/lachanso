/**
 * Suspicious Pattern Detector for Web Fraud
 * Identifies common fraud patterns in website content and behavior
 */
...
// Urgency trigger patterns (common in fraud)
...
// Fear/emotional trigger patterns (common in phishing)
const FEAR_PATTERNS = [
...
// Financial/greed patterns (common in investment scams)
...
// Conspiracy/credibility patterns (common in scams)
...
// Pressure/social engineering patterns
...
/**
 * Detect suspicious patterns in website content
 */
export function detectSuspiciousPatterns(textContent: string, htmlContent: string): PatternDetectionResult {
  ...
  for (const { pattern, weight } of PRESSURE_PATTERNS) {
    if (pattern.test(fullContent)) {
      urgencyScore += weight / 2; // Lower weight for pressure
      detectedPatterns.push({
  ...
  // Additional analysis for cloned content
  ...
  // Check for blackmail/extortion patterns
...
export function calculateManipulationRisk(patterns: PatternDetectionResult): number {
  if (!patterns.hasRedFlags) return 0;
  
  return Math.min(100, patterns.urgencyScore + (100 - patterns.trustScore));
}

/**
 * Get pattern severity summary
 */
export function getPatternSeveritySummary(
  patterns: PatternDetectionResult
): "none" | "low" | "medium" | "high" {
  if (patterns.detectedPatterns.length === 0) return "none";
