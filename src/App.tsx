...
import { checkFraudDatabase } from "./constants/fraudDatabase";
...
export default function App() {
  ...
  const handleCheck = (e?: FormEvent) => {
    ...
    setTimeout(async () => {
      ...
      // Kiểm tra trùng khớp chính xác hoặc khớp phần đuôi (ví dụ: sub.sunwin.co)
      const isGamblingScam = GAMBLING_DOMAINS.has(domainOnly) ||
        Array.from(GAMBLING_DOMAINS).some(g => domainOnly.endsWith("." + g) || domainOnly === g);
      ...
      if (links.length > 0) {
        for (const link of links) {
          ...
          const linkFraudMatches = await checkFraudDatabase(linkDomain);
      ...
      if (mentionsAuthority && !hasGovLink && !hasTrustedLink) {
        ...
        reasons.push({
          ...
          detail: "Văn bản tự xưng cơ quan nhà nước nhưng không cung cấp liên kết chính thống (.gov.vn). Đây là hành vi lừa đảo phổ sầm.",
          status: "danger",
          icon: Landmark
        });
      }
