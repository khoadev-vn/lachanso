  ...
  const handleCheck = (e?: FormEvent) => {
    ...
    setTimeout(async () => {
      ...
      function isSuspiciousTLD(url: string): boolean {
        ...
        const risktlds = [".xyz", ".top", ".online", ".club", ".vip", ".win", ".bid", ".icu", ".site", ".cc", ".cx", ".bet"];
        return risktlds.some(tld => domain.endsWith(tld));
      }
      ...
      function checkFraudDatabase(domain: string): FraudDatabaseMatch[] {
        const domainLower = domain.toLowerCase().trim();
        const isGambling = GAMBLING_DOMAINS.has(domainLower) ||
          Array.from(GAMBLING_DOMAINS).some(g => domainLower.endsWith("." + g) || domainLower === g);

        if (isGambling) {
          return [{
            isBlacklisted: true,
            database: "Danh sách đen Cờ bạc lừa đảo",
            fraudType: "gambling",
            description: "Tên miền đã bị định danh là cờ bạc trái phép hoặc lừa đảo tài chính."
          }];
        }
        return [];
      }
      ...
      async function analyzeWebsite(url: string): Promise<WebAnalysisResult> {
        const domain = getDomainOnly(url);
        const isGambling = GAMBLING_DOMAINS.has(domain) ||
          Array.from(GAMBLING_DOMAINS).some(g => domain.endsWith("." + g) || domain === g);
        ...
        if (isGambling) {
          return {
            ...
            description: "Hệ thống phát hiện đây là trang cờ bạc hoặc giả mạo giao dịch lừa đảo chiếm đoạt tài khoản.",
            ...
            reasons: [
              {
                ...
                detail: `Tên miền nằm trong danh sách đen lừa đảo cờ bạc của Lá Chắn Số.`,
                status: "danger",
                icon: AlertTriangle
              }
            ]
